...
func main() {
	...
	if metadataServiceType == "sqlite" {
		sqlDatabase := &web.SQLiteVideoMetadataService{}
		err = sqlDatabase.Initialize(metadataServiceOptions)
		if err != nil {
			fmt.Printf("Failed to initialize SQL DB: %v\n", err)
			return
		}
		defer sqlDatabase.Close()
		metadataService = sqlDatabase
	} else {
		fmt.Println("Error invalid METADATA_OPTIONS:", metadataServiceOptions)
		return
	}
	...
	if contentServiceType == "fs" {
		fileSystem := &web.FSVideoContentService{}
		err = fileSystem.Initialize(contentServiceOptions)
		if err != nil {
		...
		storageAddrs := addresses[1:]
		fileSystem, err := web.NewNetworkVideoContentService(adminAddr, storageAddrs)
		if err != nil {
			fmt.Println(err)
			return
		}
		...
		proto.RegisterVideoContentAdminServiceServer(grpcServer, fileSystem)
	} else {
		fmt.Println("Error invalid CONTENT_OPTIONS:", contentServiceOptions)
		return
	}
